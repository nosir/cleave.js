...
var selectCountryCoverTitle = DOM.select(
  ".region-selector .select-cover .title"
);
var codePhone = DOM.select(".code-phone");
var inputPhone = DOM.select(".input-phone");

for (
  var i = 0, html = '<select class="select-box">', iMax = countries.length;
  i < iMax;
  i++
) {
  var now = countries[i];
...
var selectCountry = DOM.select(
  ".region-selector .select-box-wrapper .select-box"
);
...
var selectNumeralCoverTitle = DOM.select(
